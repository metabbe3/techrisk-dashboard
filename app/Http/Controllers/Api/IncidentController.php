...
class IncidentController extends Controller
{
    ...
    {
        $cacheKey = 'incidents.' . md5(json_encode($request->all()));

        $incidents = Cache::tags(['incidents'])->remember($cacheKey, 60, function () use ($request) {
            $query = Incident::with(['labels']); // Remove incidentType from with clause
            ...
            if ($request->filled('type')) {
                $query->where('incident_type', $request->input('type')); // Directly filter by incident_type
            }

            return IncidentApiResource::collection($query->paginate(15));
        });

        return $this->successResponse($incidents, 'Incidents retrieved successfully.');
    }

    // New method to get all labels
    ...
    // New method to get all incident types
    ...
    public function store(Request $request)
    {
        ...
        return $this->successResponse(new IncidentApiResource($incident), 'Incident created successfully.', 201);
    }

    public function show(Incident $incident)
    {
        return $this->successResponse(new IncidentApiResource($incident->load([
            'pic', 
            'statusUpdates', 
    ...
    public function update(Request $request, Incident $incident)
    {
        ...
        return $this->successResponse(new IncidentApiResource($incident), 'Incident updated successfully.');
    }
